...
func AddTransaccionVinculadoRegistrarNota(m *TrVinculadoRegistrarNota) (alerta []string, err error) {
	...
	if(m.VinculacionTrabajoGrado.RolTrabajoGrado.Id == 1){
		...
		//Si es evaluador se busca la cantidad de evaluadores registrados
		var vinculacionesTG []VinculacionTrabajoGrado
		if _, err := o.QueryTable(new(VinculacionTrabajoGrado)).RelatedSel().Filter("rol_trabajo_grado",3).Filter("Activo", true).Filter("trabajo_grado",m.TrabajoGrado.Id).All(&vinculacionesTG); err == nil{
