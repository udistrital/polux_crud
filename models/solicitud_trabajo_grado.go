...
func GetSolicitudesByUser(codigo, documento string, codigoCarrera string) (solicitudes []SolicitudTrabajoGrado, err error) {
	...
	solicitudes = []SolicitudTrabajoGrado{}
	if len(trabajos) > 0 {
		qs := o.QueryTable(new(SolicitudTrabajoGrado)).RelatedSel()
		qs = qs.Filter("TrabajoGrado__in", trabajos).OrderBy("-Id")
